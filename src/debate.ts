...
function buildTurnPrompt(args: {
  ...
  return [
    ...
    args.moderatorInstruction
      ? `Moderator Instruction: ${args.moderatorInstruction}`
      : 'Moderator Instruction: Stay rigorous and factual.',
...
function buildModeratorSummary(topic: string, transcript: DebateTurn[]): string {
  ...
  const lastA =
    [...okTurns].reverse().find((t) => t.speaker === 'A')?.text ?? 'No final point from A.'
  const lastB =
    [...okTurns].reverse().find((t) => t.speaker === 'B')?.text ?? 'No final point from B.'
